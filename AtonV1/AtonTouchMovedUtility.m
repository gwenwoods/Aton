...
+(void) moveTouchElement:(UIEvent *)event:(AtonTouchElement*) touchElement:(UIView*) baseView {
    ...
    if ([self isWithinImgViewNeighborhood:touchLocation:[touchElement touchIV]]) {
...
+(BOOL) isWithinImgViewNeighborhood:(CGPoint) point:(UIImageView*)iv1 {
	// check if iv0 is within iv1
	int x0 = point.x;
	int y0 = point.y;
	...
	int x1 = iv1.center.x;
	int y1 = iv1.center.y;
	int w1 = [iv1 frame].size.width;
	int h1 = [iv1 frame].size.height;
